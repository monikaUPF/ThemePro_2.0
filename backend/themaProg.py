            ...
            if theme:
                if previousTheme:
                    if previousPron:
                        ...
                        if not pron:
                            if themeVector and previousTheme:
                                dTheme = self.embeddings.distance(themeVector, previousTheme)[0][0]
                                print("T/T Distance:",theme,previousThemeStr,dTheme)

                            else:
                                dTheme = -10

                            if themeVector and hyperThemeVector:
                                dHyper = self.embeddings.distance(themeVector, hyperThemeVector)[0][0]
                                print("T/H Distance:",theme,hyperTheme,dHyper)
                            else:
                                dHyper = -10
                        else:
                            dHyper = -10
                ...
                if previousRheme:
                    if not pron:
                        if themeVector and previousRheme:
                            dRheme = self.embeddings.distance(themeVector, previousRheme)[0][0]
                            print("T/R Distance:",theme,previousRhemeStr,dRheme)
                        else:
                            dRheme = -10
                    else:
                        dRheme = -10
